...
class BlogCategory(TranslatableModel):
    ...
    def get_absolute_url(self):
        ...
        if self.has_translation(lang):
            ...
            return reverse('djangocms_blog:posts-category',
...
class Post(ModelMeta, TranslatableModel):
    ...
    translations = TranslatedFields(
        ...
        meta_description=models.TextField(verbose_name=_(
            u'post meta description'),
                                          blank=True, default=''),
        meta_keywords=models.TextField(verbose_name=_(u'post meta keywords'),
                                       blank=True, default=''),
        meta_title=models.CharField(
            verbose_name=_(u'post meta title'),
            help_text=_(
                u'used in title tag and social sharing'),
            max_length=255,
            blank=True, default=''),
    ...
    def get_absolute_url(self):
        kwargs = {'year': self.date_published.year,
                  ...
                  'slug': self.safe_translation_getter(
                      'slug', language_code=get_language(), any_language=True)}
...
class BasePostPlugin(CMSPlugin):
    ...
    def post_queryset(self, request=None):
        ...
        posts = Post._default_manager.active_translations(
            language_code=language)
        if not request or not getattr(
                request, 'toolbar', False) or not request.toolbar.edit_mode:
...
class LatestPostsPlugin(BasePostPlugin):
    ...
    latest_posts = models.IntegerField(
        ...
        help_text=_(
            u'The number of latests articles to be displayed.'))
    ...
    categories = models.ManyToManyField(
        ...
        help_text=_(
            u'Show only the blog articles tagged with chosen categories.'))
    ...
    def get_authors(self):
        ...
        for author in authors:
            ...
            if author.djangocms_blog_post_author.filter(publish=True).exists():
                author.count = author.djangocms_blog_post_author.filter(
                    publish=True).count()
        return authors
